...
if (class_exists('Klick_Ats_Abstract_Notice')) return;

/**
 * Class Klick_Ats_Abstract_Notice
 */
abstract class Klick_Ats_Abstract_Notice {
	...
	/**
	 * Klick_Ats_Abstract_Notice constructor
	 */
	...
	public function do_notice() {
		...
		return Klick_Ats()->get_dashboard()->include_template('notices-templates/' . $this->notice_template_file, false, $notice_data);
	}
	...
	public function available_at_this_time() {
		...
		$display_notice_time = Klick_Ats()->get_options()->get_option('notice-display-time');
		...
		if (!isset($display_notice_time[$this->notice_id]) || time() > $display_notice_time[$this->notice_id]) {
				...
				Klick_Ats()->get_options()->update_option('notice-display-time', $display_notice_time);
				return true;
		} else {
				return false;
		}
	...
	public function is_plugin_configured($plugin) {
		...
		if (!function_exists('get_themes')) include_once(ABSPATH . 'wp-admin/includes/plugin.php');
		
		if (!is_plugin_active($plugin)) return false;
		
		// specific tests for specific plugins
		return ($plugin === "Advanced-theme-search/advanced-theme-search.php" && Klick_Ats()->get_options()->get_option('send-url'));
	}
}
